        ...
        dref.child("User").child(user.getUid()).child("Date").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (count_move <= 10 && count_move > 4) {
                    ...
                    addNotification("ผ่านมา 6 ชั่วโมงแล้ว", "ตอนนี้ลูกของคุณดิ้น " + count_move + " ครั้ง แนะนำให้คุณแม่นับการดิ้นของลูกต่อจนถึงเวลาต่อไป",HomeActivity.class);
                    stopSelf();
                    running = false;
                    return;
                } else {
                    Log.i("count move on Another", "is" + count_move);
                    addNotification("ผ่านมา 6 ชั่วโมงแล้ว", "ตอนนี้ลูกของคุณดิ้น " + count_move + " ครั้ง ซึ่งถือว่าอาจเกิดอันตรายหรือมีปัญหากับทารกในครรภ์ แนะนำให้คุณแม่ไปโรงพยาบาลทันทีที่ห้องคลอด รพ.จุฬาลงกรณ์ สภากาชาดไทย อาคารภูมิสิริมังคลานุสรณ์ชั้น 9 โซนเอ หรือ โทร 02-2564000 ต่อ 80921,80922",ContractActicity.class);
                    stopSelf();
                    running = false;
                    return;
                }
